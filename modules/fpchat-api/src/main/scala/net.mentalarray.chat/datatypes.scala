...
case class Username(value: String) {
  ...
  override def equals(obj: Any) = obj match {
    case Username(otherValue) => otherValue.toLowerCase == value.toLowerCase
    case _                    => false
  }

  override def hashCode() = 1771 ^ value.toLowerCase.hashCode
}

object Username {

  implicit val eqUsername: Eq[Username] = Eq.instance[Username]((a, b) => a.value.toLowerCase == b.value.toLowerCase)

  implicit val ordUsername: Ordering[Username] = Ordering.by((u: Username) => u.value.toLowerCase)
}
...
object Colors {
  ...
  def fromName(name: String) = name match {
    ...
    case "lightred"      => DIM_RED
    case "lightgreen"    => DIM_GREEN
    case "lightyellow"   => DIM_YELLOW
    case "lightblue"     => DIM_BLUE
    case "lightmagenta"  => DIM_MAGENTA
    case "lightcyan"     => DIM_CYAN
    case _               => DIM_WHITE
  }
